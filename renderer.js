...
saveChatButton.addEventListener('click', () => { currentChatId = Date.now(); saveCurrentChat(); });
newChatButton.addEventListener('click', newChat);
addPromptRowButton.addEventListener('click', () => {addPromptRow();focusLastPrompt();});
...
function handlePaste(event) {
    ...
    const preprocessedText = pastedText.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>').replace(/ /g, '&nbsp; ');
...
function deleteChat(chatId) {
    ...
    const chatElement = document.querySelector(`#saved-chats li[data-id="${chatId}"]`);
    if (chatElement) {
      chatElement.remove();
    }
    newChat();
  ...
  function saveCurrentChat() {
    ...
    const prompts = Array.from(promptContainer.querySelectorAll('.editable-prompt'));
    const chatContent = prompts.map((prompt) => {
      const type = prompt.dataset.type;
      const text = prompt.innerHTML.trim().replace(/<br>/g, '\n').replace(/<div>/g, '\n').replace(/<\/div>/g, '');
      console.log(text);
      return { type, text };
    });
    ...
    const chat = {
      ...
      name: (chatContent[0].text.substring(0, 20))?chatContent[0].text.substring(0, 20):'Untitled',
      content: chatContent,
    };
    ...
    let savedChats = JSON.parse(localStorage.getItem('saved_chats')) || [];
      ...
      const chatIndex = savedChats.findIndex((savedChat) => savedChat.id === currentChatId);
      if (chatIndex !== -1) {
  ...
  function loadSelectedChat(chatId) {
      ...
      if (selectedChat) {
          ...
          selectedChat.content.forEach((item) => {
              ...
              lastPromptDiv.innerHTML = item.text.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>').replace(/  /g, '&nbsp; ');
...
function escapeHtml(unsafe) {
  ...
  function addPromptRow(promptType) {
    ...
    label.textContent = newPromptType.charAt(0).toUpperCase() + newPromptType.slice(1); + ':';
    ...
    editablePrompt.classList.add((newPromptType == "assistant" ? 'bg-blue-100' : 'x'));
...
async function submitPrompt() {
    ...
    const promptTexts = prompts.map((prompt) => {
      const type = prompt.dataset.type;
      let text = prompt.innerHTML.trim().replace(/<br>/g, '\n').replace(/<div>/g, '\n').replace(/<\/div>/g, '');
    ...
    if (prompt) {
      ...
      try {
        const response = await fetch('https://api.anthropic.com/v1/complete', requestOptions);
        ...
        if (response.ok) {
          ...
          while (true) {
            ...
            if (done) break;
            let lastLine = ""
            let lastLineData = decoder.decode(value);
            if (lastLine.indexOf("\n")!=-1){
            ...
            if (lastLine.startsWith('data:') && lastLine.indexOf("[DONE]")==-1) {
                try{
                    const eventData = JSON.parse(lastLine.slice(5));
                    if (eventData.completion) {
                      const sanitizedCompletion = escapeHtml(eventData.completion).replace(/\t/g, '    ');
                      lastPromptDiv.innerHTML = sanitizedCompletion.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>').replace(/  /g, '&nbsp; ');
